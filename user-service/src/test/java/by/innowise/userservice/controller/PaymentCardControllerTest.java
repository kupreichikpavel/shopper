...
class PaymentCardControllerTest {
    ...
    void shouldCreatePaymentCard() throws Exception {
        ...
        when(paymentCardService.create(
                eq(USER_ID),
                any(PaymentCardCreateDto.class)
    ...
    void shouldFindAllPaymentCardsByUserId()
        ...
        when(paymentCardService.findAllByUserId(USER_ID))
                .thenReturn(List.of(createResponseDto()));
    ...
    void shouldUpdatePaymentCard() throws Exception {
        when(paymentCardService.update(
                eq(CARD_ID),
                any(PaymentCardUpdateDto.class)
        )).thenReturn(createResponseDto());
    ...
    void shouldDeactivatePaymentCard() throws Exception {
        PaymentCardResponseDto inactiveCard =
        ...
        mockMvc.perform(
                        patch(
                                "/api/v1/payment-cards/{id}/active",
                                CARD_ID
                        )
                                .contentType("application/json")
                                .content("""
    ...
    void shouldRejectInvalidCreateRequest()
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.fieldErrors.number").exists()
                )
                .andExpect(
                        jsonPath("$.fieldErrors.holder").exists()
                )
                .andExpect(
    ...
    void shouldReturnConflictWhenCardLimitExceeded()
        ...
        when(paymentCardService.create(
                ...
                new PaymentCardLimitExceededException(
                        USER_ID
                )
        );
